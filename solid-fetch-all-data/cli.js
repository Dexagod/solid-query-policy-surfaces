...
function handleAction() {
    return __awaiter(this, void 0, void 0, function () {
        var questions, answers, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    questions = [];
                    if (!options.root)
                        questions.push({ type: 'input', name: 'root', message: 'Pod root to retrieve data from' });
                    if (!options.token)
                        questions.push({ type: 'input', name: 'token', message: 'Token file location' });
                    if (!options.out)
                        questions.push({ type: 'input', name: 'out', message: 'Directory to write data to' });
                    if (!options.meta)
                        questions.push({ type: 'confirm', name: 'meta', message: 'Include metadata resources' });
                    if (!questions.length) return [3 /*break*/, 2];
